...
CREATE TABLE IF NOT EXISTS `institutions` (
  ...
  KEY `idx_institutions_slug` (`slug`)
...
CREATE TABLE IF NOT EXISTS `qr_scan_logs` (
  ...
  `updated_at` timestamp NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP,
  `deleted_at` timestamp NULL DEFAULT NULL,
  `search_type` enum('direct','universal') DEFAULT 'direct' COMMENT 'Tipo de acceso: directo por QR específico o búsqueda universal',
  `search_query` varchar(255) DEFAULT NULL COMMENT 'Serial number buscado en caso de búsqueda universal',
  `institution_id` bigint unsigned DEFAULT NULL COMMENT 'Institución desde la cual se realizó la búsqueda universal',
  PRIMARY KEY (`id`),
  KEY `qr_code_id` (`qr_code_id`),
  KEY `idx_qr_scan_logs_ip` (`ip`),
  KEY `idx_qr_scan_logs_scanned_at` (`scanned_at`),
  KEY `idx_qr_scan_logs_search_type` (`search_type`),
  KEY `idx_qr_scan_logs_search_query` (`search_query`),
  KEY `idx_qr_scan_logs_institution` (`institution_id`),
  KEY `idx_qr_scan_logs_type_institution` (`search_type`,`institution_id`),
  CONSTRAINT `fk_qr_scan_logs_institution` FOREIGN KEY (`institution_id`) REFERENCES `institutions` (`id`) ON DELETE SET NULL,
